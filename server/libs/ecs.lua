...
function emeta.__index.get(id)
    local e = {}
    for i, c in pairs(component) do e[i] = cmeta.__index[i][id] end
    return e
end

function emeta.__index.len() return emeta.used end
    ...
    setmetatable(components, {
        ...
        __call = function(self, init)
            ...
            for _, c in ipairs(self) do
                if init[c] == nil then
                    error(
                        "entity: <" .. name .. "> requires component <" .. c ..
                            ">", 2)
                end
...
function smeta:__call(...)
    ...
    for _, system in pairs(systems) do system() end
end

function smeta:__newindex(name, components)
    -- create system key
    local syskey = {}
    for _, c in ipairs(components) do syskey[#syskey + 1] = c end
    table.sort(syskey)
    ...
    for i = 1, #components do
        if component[components[i]] == nil then
            error("system: <" .. name .. "> component <" .. components[i] ..
                      "> is not defined", 2)
        end
